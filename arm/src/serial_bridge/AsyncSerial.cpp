...
class AsyncSerialImpl : private boost::noncopyable {
 public:
  ...
  std::string readBufferAccumulator;  ///< accumulator to search for callback event delimiter
  std::string delimiter_; ///< delimiter string (serial data is sent to callback after the delimiter has been reached e.g. NMEA \r\n)
  bool connected_; ///< client connection status (discard all serial data if no client is connceted)
...
//buffer the serial input and only run the callback after a delimiter has been reached
//(e.g. only invoke callback after a line has been read --> delimiter = '\n')
void AsyncSerial::readEnd(const boost::system::error_code& error,
                          size_t bytes_transferred) {

  //discard serial data if no client is connected to the serial server
  if(!connected_)
    return;
  ...
  if (error) {
    ...
    if(pimpl->delimiter_.length()>0)
    {
      //append new serial data to accumulation buffer
      pimpl->readBufferAccumulator += std::string(pimpl->readBuffer, pimpl->readBuffer+bytes_transferred);

      //split the accumulation buffer using a specified delimiter and send each token over the network
      size_t pos = 0;
      std::string token;

      while ((pos = pimpl->readBufferAccumulator.find(pimpl->delimiter_)) != std::string::npos)
      {
        //read the token (and re-add delimiter)
        token = pimpl->readBufferAccumulator.substr(0, pos) + pimpl->delimiter_;

        //send over network
        if (pimpl->callback)
          pimpl->callback(token.c_str(), token.size());

        //remove the token from the buffer
        pimpl->readBufferAccumulator.erase(0, pos + pimpl->delimiter_.length());
      }

      //prevent buffer overflows if delimiter not found (just send out data after it has reached a threshold limit)
      if(pimpl->readBufferAccumulator.size() > 2*readBufferSize)
        if (pimpl->callback)
        {
          pimpl->callback(pimpl->readBufferAccumulator.c_str(), pimpl->readBufferAccumulator.size());
          std::cout << "WARNING: Flushing serial buffer as no delimiter has been found !" << "\n";
        }
    } else {
      //just send out the serial data if no delimiter is configured
      pimpl->callback(pimpl->readBuffer, bytes_transferred);
      pimpl->readBufferAccumulator.clear();
    }
    //read some more data from serial port
    doRead();
  }
}
...
void AsyncSerial::setReadDelimiter(std::string delim)
{
  pimpl->delimiter_ = delim;
}
/// ser client connection status
void AsyncSerial::setClientConnectionState(bool connected)
{
  //set the status
  pimpl->connected_=connected;
  //flush all serial buffers
  pimpl->readBufferAccumulator.clear();
  std::cout << "Serial server: flushing serial buffers (client dis/connected)\n";
}
//
...
void CallbackAsyncSerial::setDelimiter(std::string delim)
{
  setReadDelimiter(delim);
}

void CallbackAsyncSerial::setClientConnectionStatus(bool connected)
{
