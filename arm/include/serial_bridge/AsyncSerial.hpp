 ...
 *         extended: Thomas Schneider (2014)
...
class AsyncSerial : private boost::noncopyable {
 ...
 private:
  /**
   * Client connection status (if no client connected discard all data)
   */
  bool connected_;
  /**
 ...
 protected:
  ...
  /**
   * To allow derived classes to set a read delimiter
   */
  void setReadDelimiter(std::string delim);
  /**
   * Set client connection state
   */
  void setClientConnectionState(bool connected);
  /**
...
class CallbackAsyncSerial : public AsyncSerial {
 public:
  ...
  void setClientConnectionStatus(bool connected);

  typedef boost::shared_ptr<CallbackAsyncSerial> Ptr;
};
#endif //ASYNCSERIAL_H
